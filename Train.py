...
class custom_dset(data.Dataset):
    def __init__(self, train, train_label, batch_size):
        ...
        label_set = set(self.train_label)
        self.label_set = set([torch.from_numpy(numpy.array(label_set[i])).type(torch.LongTensor) for i in range(len(label_set))])
        self.map = defaultdict(list)
        for i in range(len(self.train)):
            self.map[self.train_label[i]].append(self.train[i])
    ...
    def __getitem__(self, index):
        ...
        # mock printed image
        # train_setting_p = torch.ones_like(train_setting)
        train_setting_p_idx = -1
        while train_setting_p_idx != index:
            train_setting_p_idx = random.randint(0, len(self.map[label_setting])-1)
        train_setting_p = self.map[label_setting][train_setting_p_idx]
        train_setting_p = train_setting_p.view(1, size[2], size[3])

        negative_idx_list = [i for i in range(len(self.label_set)) if self.label_set[i] != label_setting]
        label_setting_n_idx = random.choice(negative_idx_list)
        label_setting_n = self.label_set[label_setting_n_idx]
        train_setting_n_idx = random.randint(0, len(self.map[label_setting_n])-1)
        train_setting_n = self.map[label_setting_n][train_setting_n_idx]
        train_setting_n = train_setting_n.view(1, size[2], size[3])

        label_setting = label_setting.view(-1)
        label_setting_n = label_setting_n.view(-1)
        return train_setting, label_setting, train_setting_p, train_setting_n, label_setting_n
...
def collate_fn(batch):
    ...
    img, label, img_p, img_n, label_n = zip(*batch)
    aa1 = 0
    bb1 = 0
    k = 0
    k1 = 0
    max_len = len(label[0])+1
    max_len_n = len(label_n[0])+1
    ...
    k = 0
    for ii in img_p:
        img_mask_sub_padding_p = padding_mask(ii, aa1, bb1)
        if k==0:
    ...
    k = 0
    for ii in img_n:
        img_mask_sub_padding_n = padding_mask(ii, aa1, bb1)
        if k==0:
            img_padding_mask_n = img_mask_sub_padding_n
        else:
            img_padding_mask_n = torch.cat((img_padding_mask_n,img_mask_sub_padding_n),dim=0)
        k = k+1
    ...
    for ii1 in label_n:
        ...
        ii1 = ii1.unsqueeze(0)
        ii1_len = ii1.size()[1]
        m = torch.nn.ZeroPad2d((0,max_len_n-ii1_len,0,0))
        ii1_padding = m(ii1)
        if k1 == 0:
            label_padding_n = ii1_padding
        else:
            label_padding_n = torch.cat((label_padding_n,ii1_padding),dim=0)
        k1 = k1+1


    img_padding_mask = img_padding_mask/255.0
    img_padding_mask_n = img_padding_mask_n/255.0
    img_padding_mask_p = img_padding_mask_p/255.0
    return img_padding_mask, label_padding, img_padding_mask_p, label_padding_p, img_padding_mask_n, label_padding_n
...
def my_train(target_length, target_length_p, target_length_n, attn_decoder1, discriminator,
             output_highfeature, output_highfeature_p, output_highfeature_n, output_area, output_area_p, output_area_n, y, y_p, y_n, criterion, criterion_mse,encoder_optimizer1,decoder_optimizer1,x_mean, x_mean_p, x_mean_n, dense_input, dense_input_p, dense_input_n, h_mask, h_mask_p, h_mask_n, w_mask, w_mask_p, w_mask_n, gpu,
             decoder_input, decoder_input_p, decoder_input_n, decoder_hidden, decoder_hidden_p, decoder_hidden_n, attention_sum, attention_sum_p, attention_sum_n, decoder_attention, decoder_attention_p, decoder_attention_n):
    loss = 0
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            decoder_output, decoder_hidden, decoder_attention, attention_sum, out_emb, context = attn_decoder1(decoder_input,
                                                                                             decoder_hidden,
                                                                                             output_highfeature,
                                                                                             output_area,
                                                                                             attention_sum,
            ...
            decoder_output_p, decoder_hidden_p, decoder_attention_p, attention_sum_p, out_emb_p, context_p = attn_decoder1(decoder_input_p,
                                                                                             decoder_hidden_p,
                                                                                             output_highfeature_p,
                                                                                             output_area_p,
                                                                                             attention_sum_p,
                                                                                             decoder_attention_p,
                                                                                             dense_input_p,batch_size,h_mask_p,w_mask_p,gpu)
            decoder_output_n, decoder_hidden_n, decoder_attention_n, attention_sum_n, out_emb_n, context_n = attn_decoder1(decoder_input_n,
                                                                                             decoder_hidden_n,
                                                                                             output_highfeature_n,
                                                                                             output_area_n,
                                                                                             attention_sum_n,
                                                                                             decoder_attention_n,
                                                                                             dense_input_n,batch_size,h_mask_n,w_mask_n,gpu)
            ...
            for i in range(batch_size):
                if int(y[0][i][di]) == 0:
                    ...
                    if flag_z[i] > 1:
                        ...
                        loss += criterion(decoder_output[i], y[:,i,di]) + criterion(decoder_output_p[i], y_p[:,i,di]) + criterion(decoder_output_n[i], y_n[:,i,di]) + criterion_mse(context, context_p) - criterion_mse(context, context_n)
                else:
                    loss += criterion(decoder_output[i], y[:,i,di]) + criterion(decoder_output_p[i], y_p[:,i,di]) + criterion(decoder_output_n[i], y_n[:,i,di]) + criterion_mse(context, context_p) - criterion_mse(context, context_n)
        ...
        my_num = 0
        for di in range(target_length):
            decoder_output, decoder_hidden, decoder_attention,attention_sum, out_emb, context = attn_decoder1(decoder_input, decoder_hidden,
...
decoder_input_init_n = torch.LongTensor([111]*batch_size).cuda()
...
decoder_hidden_init_n = torch.randn(batch_size, 1, hidden_size).cuda()
nn.init.xavier_uniform_(decoder_hidden_init)
nn.init.xavier_uniform_(decoder_hidden_init_p)
nn.init.xavier_uniform_(decoder_hidden_init_n)
...
for epoch in range(200):
    ...
    # this is the train
    for step,(x,y,x_p,y_p,x_n,y_n) in enumerate(train_loader):
        ...
        for (i,j,k) in zip(x,x_p,x_n):
            ...
            size_mask_n = k[1].size()
            s_w_n = str(k[1][0])
            s_h_n = str(k[1][:,1])
            w_n = s_w_n.count('1')
            h_n = s_h_n.count('1')
            h_comp_n = int(h_n/16)+1
            w_comp_n = int(w_n/16)+1
            h_mask_n.append(h_comp_n)
            w_mask_n.append(w_comp_n)
        ...
        output_highfeature_p = encoder(x_p)
        output_highfeature_n = encoder(x_n)
        ...
        for i in output_highfeature_p:
            x_mean_p.append(float(torch.mean(i)))
        for i in output_highfeature_n:
            x_mean_n.append(float(torch.mean(i)))
        ...
        for i in range(batch_size):
            ...
            decoder_hidden_init_p[i] = decoder_hidden_init_p[i]*x_mean_p[i]
            decoder_hidden_init_p[i] = torch.tanh(decoder_hidden_init_p[i])

            decoder_hidden_init_n[i] = decoder_hidden_init_n[i]*x_mean[i]
            decoder_hidden_init_n[i] = torch.tanh(decoder_hidden_init_n[i])
            # print(decoder_hidden_init[i].size())
        ...
        output_area1_n = output_highfeature_n.size()

        output_area = output_area1[3]
        output_area_p = output_area1_p[3]
        output_area_n = output_area1_n[3]

        dense_input = output_area1[2]
        dense_input_p = output_area1_p[2]
        dense_input_n = output_area1_n[2]

        target_length = y.size()[1]
        target_length_p = y_p.size()[1]
        target_length_n = y_n.size()[1]
        ...
        attention_sum_init_n = torch.zeros(batch_size,1,dense_input_n,output_area_n).cuda()
        decoder_attention_init_n = torch.zeros(batch_size,1,dense_input_n,output_area_n).cuda()
        running_loss += my_train(target_length, target_length_p, target_length_n, attn_decoder1, discriminator, output_highfeature, output_highfeature_p, output_highfeature_n,
                                output_area, output_area_p, output_area_n, y, y_p, y_n, criterion, criterion_mse,encoder_optimizer1,decoder_optimizer1,x_mean, x_mean_p, x_mean_n, dense_input, dense_input_p, dense_input_n,h_mask, h_mask_p, h_mask_n, w_mask, w_mask_p, w_mask_n,gpu,
                                decoder_input_init, decoder_input_init_p, decoder_input_init_n, decoder_hidden_init, decoder_hidden_init_p, decoder_hidden_init_n,attention_sum_init, attention_sum_init_p, decoder_attention_init_n,decoder_attention_init, decoder_attention_init_p, decoder_attention_init_n)
